...
//router.get('/',handler1, handler2 ,async (req, res) => {
  const router_getAll_users:RequestHandler=async (req, res)=>{
    try {
  ...
  const router_getUserById:RequestHandler=async (req, res)=>{
    try {
      console.log('---- DENTRO DE /API/USER ------');
  ...
  const router_Register_User:RequestHandler = async (req, res)=>{   
    ...
    //  active: true
     ...
     // console.log('---- ------------------------EMAIL DUPLICADO------------------------------------------------------------∫---:',emailduplicado);
      if (emailduplicado.length==0) {res.status(500).send({ error: 'El email está duplicado' });}
...
const createToken:Function = async (user:TipoUser) => {
...
const router_Login:RequestHandler=async (req, res) => {
...
const router_update_Admins:RequestHandler<{adminid:string}>=async (req, res) => {
...
const router_update_Students:RequestHandler<{studentid:string}>=async (req, res) => {
  // const [result] = await update_users(req.params.user_id, req.body);
   //en req.body van los datos de admin leidos de la pantalla (menos id y el admin_user_id que es el foreign key)
   // DEL FORMULARIO NOS ENVIA en req.body -> {admin_name,admin_zip,admin_address,admin_phone,admin_city}
    try {
...
const router_update_Teachers:RequestHandler<{teachid:string}>=async (req, res) => {
  // const [result] = await update_users(req.params.user_id, req.body);
...
// PATCH http://localhost:3333/api/users/23
  const router_setdeleted_user_ById:RequestHandler<{user_id:string}>=async (req, res) => {
    try {
