...
const insert_users = async ({ user_email, user_password, user_type, user_deleted }) => {
...
const update_users = async (user_id, { user_type, user_deleted }) => {
...
const setdeleted_user_ById = async (user_id) => {
    return await db.query('update Appteachers.Users set user_deleted = "SI" where user_id = ?', [user_id]);
};
// esta función no la usamos:
const delete_user_ById = async (user_id) => {
    return await db.query('delete from Appteachers.Users where user_id = ?', [user_id]);
};
module.exports = {
    getAll_users,
    insert_users,
